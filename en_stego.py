...
def hide(parent_image_path, child_image_path):
    ...
    if not __is_large(parent_image_path, child_image_path):
    ...
    for i in range(parent_image.shape[0]):
        for j in range(parent_image.shape[1]):
            parent_rgb = __int_to_bin(parent_image[i, j])

            # Use a black pixel as default
            child_rgb = __int_to_bin((0, 0, 0))

            # Check if the pixel map position is valid for the second image
            if i < child_image.shape[0] and j < child_image.shape[1]:
                child_rgb = __int_to_bin(child_image[i, j])

            # Merge the two pixels and convert it to a integer tuple
            rgb = __merge_rgb(parent_rgb, child_rgb)

            en_image[i, j] = __bin_to_int(rgb)
